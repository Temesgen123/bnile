...
//common 
const Price = (field: string) => z.coerce.number().refine((value) => /^\d+(\.\d{2})?$/.test(formatNumberWithDecimal(value)), `${field} must have exactly two decimal places (e.g., 49.99)`);
...
export const ProductInputSchema = z.object({
    ...
    numReviews: z.coerce.number().int().nonnegative('Number of reviews must be non-negative number.'),
